...
function useFirestoreProducts(){
        ...
        query.doc(uid).get().then((doc) =>{
            if(doc.exists){
                ...
                productInfo && db.collection("users").doc(uid).set({
                    ...
                    wishlist: type === "wishlist" && !checkProductExists(wishlistData, productInfo) ? wishlistData.concat(productInfo) : wishlistData
                })
            }else{
                query.doc(uid).set({
                    cart: [],
    ...
    const removeFromFirestore = (uid, product, option) =>{
        query.doc(uid).get().then((doc) =>{
            ...
            const index = (type === "success" ? cartData: wishlistData).findIndex((item) => item.id === productInfo.id);
            type=== "success" ? cartData.splice(index, 1) : wishlistData.splice(index, 1);
